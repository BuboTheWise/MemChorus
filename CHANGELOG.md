...
### Added
- **Canonical palace-location release (closes #172):** the single shared resolver `memchorus/palace_path.py` (`palace_data_dir` / `palace_data_file` / `classify` / `migrate`) is now the ONE code path that decides where `chroma.sqlite3` lives. The MCP reader (`mempalace_memory_source` space form + equals form), the `auto_init` writer (`generate_config` now delegates `data_dir` to `palace_data_dir` instead of hardcoding `DEFAULT_DATA_DIR/profile`), and the new `memchorus-doctor --palace-layout` (`--strict`) subcommand all route through it, so writer / reader / doctor **structurally agree** on one path rather than coinciding on two independent constants. `memchorus-init --migrate <root>` is wired into `auto_init.cli_main` and dispatches to `palace_path.migrate()`, making the doctor's own durable-fix hint a runnable command. `palace_data_dir` now returns the caller's value **as-given** (str in → str out; `Path` in → `Path` out — runtime-identical for `Path` inputs) so the writer records the directory verbatim and no normalization can invent a subpath or rewrite separators. `_normalize_palace_args` is now a thin compat shim that emits exactly one WARNING on the legacy-leaf migration path. New regression locks: `tests/test_palace_canonical_by_construction.py` (writer / reader agree by construction, no rewrite, no WARNING), `tests/test_palace_path_alignment.py` (legacy sibling-leaf repoint + E2E reader-sees-writer), and `tests/test_palace_layout_doctor.py` (strict FAIL → `--migrate` → strict PASS), plus `tests/test_docs_phantom_palace_layout.py` (a grep lock that fails if the previously phantom per-profile palace layout reappears in human-facing docs). README per-profile section + `docs/REQUIREMENTS.md` layout row name the canonical leaf `profiles/<name>/.mempalace/palace/` and show `memchorus-doctor --palace-layout` as the verify command.

### Fixed
- **Windows cross-platform regression in `palace_data_dir` (closes #172 CI):** the no-descent branch previously ran `Path(root)` and returned the normalized `Path`, which on Windows rewrote a forward-slash path (`/opt/mem palace/data` → `\opt\mem palace\data`) and broke the pre-existing `tests/test_auto_init.py::test_data_dir_embedded_when_supplied` verbatim-identity assertion (Linux hides the bug because its native separator is `/`). The no-descent branch now returns the input as-given — a `str` stays a `str` — while the leaf branch still returns a real `Path` (a directory discovered on disk), and the one arithmetic consumer (`palace_data_file`) wraps in `Path(...)` so `/ CHROMA_FILE` always resolves. Windows 3.11 + 3.12 CI green.

### Changed
- Bumps `__version__` 2.0.36 → 2.0.37 (next free dual-digit patch; no collision with sibling PR #179 / #166, keeping the release chain collision-free).
