...
- `MemoryOrchestrator.mark_relevant_injected_as_useful()` / `mark_relevant_injected_as_stale()` call `HitRateTracker.record_useful()` / `record_stale()` per key in the `_recent_recall_keys` buffer — the positive/negative feedback surface (spec §10.2 step 1). Both are fully graceful: they return 0 and swallow any tracker error, and are no-ops when the buffer is empty.
- **Live caller of the feedback surface:** `MemChorusHooks.on_session_end()` is the production call site (spec §10.2 step 1 — not host-side, it runs inside the memchorus hook teardown). It runs the existing `MistakeDetector.classify_and_flag()` over the user message text collected during the session, then routes the signal through the buffer:
  - user pushed back on recalled content (`noise > 0`) → `mark_relevant_injected_as_stale()` → `record_stale()` per key
  - clean turn, no pushback (`noise == 0` and user text present) → `mark_relevant_injected_as_useful()` → `record_useful()` per key

  Both calls are wrapped in `try/except` so feedback bookkeeping never interrupts session teardown. This is the fix for the #152 review-fail: previously these methods had zero production callers and `boost_factor_for_key` stayed pinned at 1.0.
