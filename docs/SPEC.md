...
- **Known packaging gap (OPEN):** `setup.py` declares only `pydantic` + `pyyaml`; **no `opentelemetry-*` pin** even though the shared Hermes venv carries OTel and `mempalace[mcp]` pulls it transitively. A re-resolution of the shared venv can split OTel versions (1.39.1/1.44.0) and brick the `mempalace` CLI import. Interim fix applied 2026-09-02 (local re-pin to 1.44.0). **Durable fix pending (vault-first, then code cycle)**: declare or document the OTel/co-located dependency expectations.
