...
- [x] Uses standard Python libraries only — `install_requires` is now explicit: `pydantic>=2.0,<3.0` + `pyyaml>=5.4`; `mcp>=1.29,<3.0` lives under the `[mcp]` extra (audit 2026-09-02). **Known gap (OPEN):** no `opentelemetry-*` pin — see MemPalace DB Location Contract / packaging notes
