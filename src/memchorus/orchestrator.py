...
class MemoryOrchestrator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # GAP027: RecursionGuard replaces the previous boolean _in_enforcement_save /
        # _in_enforcement_recall sentinel pattern. Guard caps depth across all enforcement
        # entry points (save, retrieve, search_by_key, search). Thread-safe by default
        # via RLock inside RecursionGuard itself — no separate lock needed.
        max_enf_depth = int(self.config.get("enforcement_max_depth", 5))
        self._guard: 'RecursionGuard' = RecursionGuard(max_depth=max_enf_depth)
    ...
    def save(self,
        ...
        # GAP027: RecursionGuard protects save() from recursive enforcement when
        # capture_outcome calls back into save(). Thread-safe via RLock inside the guard.
