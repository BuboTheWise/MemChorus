...
class TestSignificanceDetection(unittest.TestCase):
    ...
    def test_learning_detected_on_architecture(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The architecture design uses a hub-and-spoke pattern for event routing",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_provides_capability(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "This module provides capability for distributed state synchronization",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_handles_requests(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The gateway handles requests from multiple concurrent clients",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_designed_to(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The pipeline is designed to process events in batches of 100",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_workflow_pattern(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The workflow design follows a publish-subscribe pattern",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_key_insight(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "Key insight from reviewing the config: defaults are overridden at runtime",
            SignificanceCategory.LEARNING,
        )

    def test_learning_detected_on_understanding_of(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "My understanding of the hook lifecycle is that it fires on every turn",
            SignificanceCategory.LEARNING,
        )

    # MISTAKE: technical error patterns and pitfalls
    def test_mistake_detected_on_fails_when(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The parser fails when the input contains unescaped quotation marks",
            SignificanceCategory.MISTAKE,
        )

    def test_mistake_detected_on_bug_in(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "There is a bug in the dedup logic that causes false positives",
            SignificanceCategory.MISTAKE,
        )

    def test_mistake_detected_on_does_not_support(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The API does not support batch operations on nested resources",
            SignificanceCategory.MISTAKE,
        )

    def test_mistake_detected_on_known_issue(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "Known issue with race conditions in the cache invalidation path",
            SignificanceCategory.MISTAKE,
        )

    def test_mistake_detected_on_causes_crash(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "Sending null values to the serialization layer causes a crash in Python 3.8",
            SignificanceCategory.MISTAKE,
        )

    def test_mistake_detected_on_pitfall(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "A common pitfall when using async generators is forgetting to await properly",
            SignificanceCategory.MISTAKE,
        )

    # DECISION: technical/architectural decision indicators
    def test_decision_detected_on_default_to(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "The config defaults to a 30-second timeout for all HTTP calls",
            SignificanceCategory.DECISION,
        )

    def test_decision_detected_on_fallback(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "Fallback to local cache when the primary service is unreachable",
            SignificanceCategory.DECISION,
        )

    def test_decision_detected_on_required_by(self) -> None:
        engine = _make_engine()
        self._assert_category(
            engine,
            "Explicit type hints are required by the linting configuration",
            SignificanceCategory.DECISION,
        )
