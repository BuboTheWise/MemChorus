...
class TestPlaceholderArtifactDetection:
    ...
    @pytest.mark.parametrize("text", [
        "session context t_ffffffff current task",
        "session context t_ffffffff current task",
        "Session Context T_17cfe174 Current Task",
        "tool output for t_ffffffff",
        "execution context t_ffffffff",
        "tool result t_ffffffff",
    ])
    ...
    def test_short_unstructured_not_placeholder_passes(self):
        """Short unstructured text that isn't a placeholder should pass — heuristic was removed due to false positives."""
        filler = "this is just some random text output from a tool call today"
        assert _is_placeholder_artifact(filler) is False
...
class TestAutoStorageIntegration:
    ...
    def test_placeholder_rejected_by_engine(self):
        ...
        result = self.engine.capture_outcome(
            "session context t_ffffffff current task", outcome_type="automatic"
        )
